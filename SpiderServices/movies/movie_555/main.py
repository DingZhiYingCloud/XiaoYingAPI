...
class Movie555Spider:
    ...
    def get_categories(self) -> dict:
        ...
        :return: {main: [{id, name}], rank: [{id, name}], labels: [{key, name}]}
        """
        def fetch():
            return {
                "main": [{"id": k, "name": v} for k, v in U.MAIN_CATEGORIES.items()],
                "rank": [{"id": k, "name": v} for k, v in U.RANK_CATEGORIES.items()],
    ...
    def get_list(self, type_id: int, page: int = 1, order: str = None, year: int = None) -> dict:
        """
        获取分类列表（分页）。

        :param type_id: 分类 id（1=电影 2=连续剧 3=综艺纪录 4=动漫，或榜单 id）
        :param page: 页码，从 1 开始
        :param order: 排序方式，可选 time(时间) / hits(人气) / score(评分)
        :param year: 年份筛选，如 2026
        :return: {type_id, page, items: [...], pagination: {current, total}}
        """
        key = f"list:{type_id}:{page}:{order}:{year}"

        def fetch():
            if year:
                url = U.build_year_url(type_id, year, page)
            elif order:
                url = U.build_sort_url(type_id, order, page)
            else:
                url = U.build_list_url(type_id, page)
