...
# detail 接口「视频不可见」时的 filter_reason 提示（[实测] 2026-09-11 匿名请求）
_FILTER_REASON_HINTS = {
    "status_audit_self_see": "视频审核中，仅作者本人可见",
    "core_dep": "视频已下架或不可访问",
}
...
class DouyinVideoAnalysis:
    ...
    def _fetch_detail(self, video_id: str) -> dict:
        ...
        for attempt in range(_DETAIL_MAX_RETRY + 1):
            ...
            if not detail:
                # detail 接口在视频不可见时返回 filter_detail.filter_reason，
                # 原先统一显示「未知原因」，无法定位；这里把原因透出来
                reason = (data.get("filter_detail") or {}).get("filter_reason") or ""
                if reason:
                    hint = _FILTER_REASON_HINTS.get(reason, "视频当前不可访问")
                    last_err = f"{hint}（{reason}）"
                else:
