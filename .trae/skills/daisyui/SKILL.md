...
## 本项目（XiaoYingAPI / Django）使用须知

在本仓库内编写 Django 模板 / HTML 时，以本项目文档为准，通用安装章节（npm/CDN）不适用于本项目：

1. **必须先阅读** `API/templates/前端开发必看.md`，本文档与之冲突时以该文档为准。
2. 本项目无 Node.js：页面引用的是 `API/static/css/output.css`（Tailwind 独立可执行文件编译产物），不要执行 `npm i daisyui`、也不要引入 CDN。
3. 新页面一律继承母版 `API/templates/template.html`（已引入 output.css），只填 `title/content/js` 等 block；公共头尾写进 `API/templates/common_html/`。
4. 新增 daisyUI / Tailwind 类名后必须重跑编译并把 output.css 一起提交（命令见 `前端开发必看.md` 第 4 节）。
5. 组件类名不确定时查 daisyui 文档 MCP（Context7）或官网，禁止编造类名；禁止引入其它 UI 框架或用全局 CSS 覆盖 daisyUI 组件。
