 ...
 *   <script>
 *     XYCaptcha.init({
 *       autoVerify: true,                  // 默认 true：客户端自动调二次校验接口并回调 onResult。
 *                                          // 传 false 时只回调 onValidate（不消耗一次性凭证），
 *                                          // 由接入方把参数交给自己的后端做二次校验（登录/注册页即如此）。
 *       onReady:    function () { ... },   // 验证码就绪时回调（可提示用户）
 *       onValidate: function (validate) { ... }, // 用户验证通过、拿到验证参数时回调（可选）
 *                                                // validate = {lot_number, captcha_output, pass_token, gen_time}
 *       onResult:   function (data) {      // 仅 autoVerify=true 时回调
 *         // 用户完成验证且服务端二次校验完成后回调
 ...
 *       onClose:    function () { ... },      // 用户主动关闭验证弹层时回调（可选）
 *     });
    ...
    global.initAlicom4({ captchaId: appId, product: 'bind' }, function (obj) {
      ...
      obj.onSuccess(function () {
        ...
        if (validate && validate.lot_number) {
          // 透传给接入方（如回填调试表单 / 自行走其它校验通道）
          if (_options && typeof _options.onValidate === 'function') {
            _options.onValidate(validate);
          }
          // autoVerify=false：交接入方后端校验（二次校验凭证是一次性的，客户端先校验会导致服务端复验失败）
      ...
      if (obj.onClose) {
        obj.onClose(function () {
          if (_options && typeof _options.onClose === 'function') { _options.onClose(); }
        });
      }
