...
(function () {
  ...
  function runEndpoint(btn) {
    ...
    resBox.innerHTML = '';
    resBox.appendChild(el('div', 'text-sm text-base-content/60 py-2', '正在请求 ' + path + ' …'));
    ...
    fetch('/docs/_call/', {
      ...
      resBox.innerHTML = '';
      resBox.appendChild(el('p', 'text-sm text-error', '网络异常，请检查网络后重试'));
    }).finally(function () {
      btn.disabled = false;
  ...
  function renderResult(resBox, data) {
    ...
    if (data.elapsed_ms !== undefined) {
      head.appendChild(el('span', 'text-xs opacity-60', '耗时 ' + data.elapsed_ms + ' ms'));
    }
    ...
    var body = parsed ? JSON.stringify(parsed, null, 2) : (data.text || '(空响应)');
